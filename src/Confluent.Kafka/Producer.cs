...
namespace Confluent.Kafka
{
    ...
    internal class Producer<TKey, TValue> : IProducer<TKey, TValue>, IClient
    {
            ...
            Message<TKey, TValue> message)
        {
            ...
            topicPartition = GetPartition(topicPartition, message, keyBytes, valBytes);
        ...
        private TopicPartition GetPartition(TopicPartition topicPartition, Message<TKey, TValue> message, byte[] keyBytes, byte[] valBytes)
        {
            var newTopicPartition = topicPartition;

            if (this.partitionerHandler != null && topicPartition.Partition == Partition.Any)
            {
                var partition = this.partitionerHandler?.Invoke(new PartitionRequest<TKey, TValue>(message, keyBytes, valBytes));
